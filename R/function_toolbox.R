...
order_data <- function(MACSQuant) {
    ...
    for (i in sorted_vector) {
        ...
        if (length(matched) != 2) {
            warning(paste("Your file do not contains 2 gates for replicate: ",
    ...
    ignored_print <- paste(unique(MACSQuant@my_data$WID[ignored]), collapse = ",")
    message("--> Done: Replicates sorted")
    ...
    if (MACSQuant@param.output$save.files == TRUE) {
        write.table(MACSQuant@my_data[indices, ], "./outputMQ/sorted_table.txt",
            sep = "\t"
        )
